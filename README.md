...
输出为 `BatchExtractionResult` 对象，顶层结构：
...
```json
{
  ...
  "results": [ ... ],
  "failed_ids": [],
  "summary": "共处理25条对话。已解决15条（60%），涉及投诉4条，需人工复核N条。"
}
```

每条对话的提取结果在 `results` 数组中，每条为 `ExtractionResult` 对象（28 个字段，覆盖 8 大维度）。参考示例见 `data/task2_extract_example.md`（仅作参考，不代表标准答案）。

Mock 模式实际运行结果：`total_count=25, success_count=25, failed_count=0`。

## Schema 设计思路

Schema 位于 `src/extractor/schema.py`，基于 **Pydantic v2**，覆盖 8 大维度共 28 个字段。
...
### 第三阶段：测试与人工抽检 ✅
- [x] pytest 21 个测试全部通过
- [x] 人工抽检 8 条对话，准确率 95.0%（38/40）

### 可选增强
- [ ] `LLMExtractor`：调用 LLM API 做语义提取，改善复杂场景准确率
- [ ] 周报自动生成模块
...
- **mock**：当前已实现，基于规则匹配，不依赖外部 API，适合复现和测试。
- **llm**：预留扩展模式，本次交付未启用。后续可接入 OpenAI-compatible API，与 mock 模式共享同一 Schema。
...
本项目使用 GitHub Copilot（Claude Code）辅助开发，分工如下：

- **AI 辅助**：项目骨架生成、mock 抽取器规则代码初稿、pytest 测试用例初稿、README 初稿、评估报告格式化。
- **人工主导**：Schema 字段体系设计、分类枚举与风险标签定义、人工复核规则制定、extract_example.md 对照分析。
- **人工抽检**：对 conv_05、conv_06、conv_09、conv_16、conv_20 等复杂样本逐条核查分类、情绪、风险标签、证据摘取质量。
- **人工修正**：修正规则误判（延迟识别、退款优先于服务响应、投诉升级风险漏判、resolution_action 简略），并同步更新评估报告。
- **验证**：通过 CLI 命令行运行 25 条对话抽取，pytest 21 个测试全部通过，人工抽检 8 条 38/40。
...
├── outputs/
│   ├── extracted_results.json      # Mock 模式输出（25 条结果）
│   └── eval_report.md              # 人工抽检报告（8 条，38/40）
├── tests/
│   ├── __init__.py
│   ├── test_schema.py              # Schema 校验测试（9 个）
│   └── test_mock_extractor.py      # Mock 抽取器测试（12 个）
├── screenshots/
│   ├── dev_process.png             # 开发过程截图
│   └── run_result.png              # 运行结果截图
├── src/
│   └── extractor/
│       ├── __init__.py
│       ├── schema.py               # Pydantic Schema 定义
│       ├── utils.py                # 通用工具函数
│       ├── mock_extractor.py       # 基于规则的抽取器
│       └── extractor.py            # CLI 入口
├── .env.example
├── .gitattributes
├── .gitignore
